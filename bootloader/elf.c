...
/*check whether elf_header is valid*/
...
static int elf_load_program(struct elf_header *header)
{
  ...
  for (i = 0; i < header->program_header_num; i++) {
    ...
    if (phdr->type != 1) /*is loadable segment*/
      continue;

    /*copy segment to physical memory*/
    memcpy((char *)phdr->physical_addr, (char *)header + phdr->offset, phdr->file_size);
    memset((char *)phdr->physical_addr + phdr->file_size, 0, phdr->memory_size - phdr->file_size); //used soon. for bss
  }
